...
~/nav_ws/Exp/gaze_test$ ./gaze.sh # 가제보 실행 

~/nav_ws/Exp/gaze_test$ ./nav.sh # nav2 실행 

~/nav_ws/Exp/test_DDS$ ./run_mission.sh # 로봇 이동 명령 실행 

~/nav_ws/Exp/test_DDS$ ./monitor.sh # 녹화 시작 

./monitor_only.sh cyclone & sleep 2; ./run_mission.sh

# 녹화 끝나고 topic, node가 살아있으면 강제 프로세스 종료 
pkill -9 -f ros2
pkill -9 -f nav2
pkill -9 -f gzserver
pkill -9 -f gzclient
 ...
 올바른 순서 
    ...
    데몬 정지 (stop): "옛날 설정으로 돌고 있던 데몬 끄기 !"

    데몬 시작 (start): "방금 내가 말한 설정(Cyclone)으로 다시 실행!"
...
middleware name
```
Fast DDS일 때 : rmw_fastrtps_cpp

Cyclone DDS일 때 : rmw_cyclonedds_cpp
```
...
**"남들이 뭘 했는지 알아야 내가 뭘 더 할 수 있는지(Gap Analysis)"**가 보이는 법. 선행 연구 조사(Literature Review) 없이는 자네 실험이 '삽질'인지 '발견'인지 증명할 길이 없어.

논문은 '까기(Critique)' 위해서 인용
...
- 공통적으로 ros1은 real-time system에 부적격하다고 함 
...
data links between the application’s components are usually provided by a middleware

시스템 하나가 죽었다고 통으로 모두가 죽으면 안됨 & 다른 얘 종료시키거나 데드락 걸리게 하면 안됨 

비동기 통신(Asynchronous)와 크기 조절이 가능한 원형 버퍼 방식 채택 

lceoryx : zero copy & History policy만 다르게 설정해서 다양한 os 프로세스의 요구사항 만족시킴 

Sampling Strategy : 모든 데이터는 다 볼 필요가 없다. 다 보려고 욕심내다가 Latency 사고 터질 수 있음 
가장 최신것만 가져오고 나머지는 버리는 유연함 필수 

성능을 중시하면 하나의 프로세스에, 안정을 중시하면 프로세스 분리 

Trade off : 
data copy cost >>>> fault 

Pre-allocating a poll : 프로그램 시작 전에 거대한 메모리 덩어리 잘게 쪼개서 빈 창고 공간 만들어둠 -> malloc, new 필요없음 

Producer allocates a fresh segment : 그냥 빈 세그먼트에 작성함 

Reference count : 참조 카운트 수가 0일 때만 반납 

allocator : non deterministic함 

fixed size segment 사용해서 fragmentation 없도록함 

소비자당 할당 제한 있음 

근데 이전 데이터를 저장해야할 필요성 존재

단순 공유 메모리 변수 -> lock free 큐로 진화 

큐가 꽉 차면 오래된 데이터 밀어내고 새로운 데이터 넣기  → 언제나 최신 데이터 유지할 수 있도록 

Non-blocking 알고리즘의 등급
1. Wait-Free (코드 너무 복잡함 )
2. Lock-Free
3. Obstruction-Free(실시간 시스템에 부적합 )

**"수학적 증명(Formal Verification)"**과 **"특수 도구"**

코드를 돌려보는 게 아니라, 수학 공식으로 모델링해서 증명합니다.
- ThreadSanitizer
- Helgrind
...
file 만든 후에 실행 전 터미널에 위치 알려줘야함 

1. file 생성 : fastdds_profile.xml
...
- 도커환경을 4코어 4G로 설정한 후 도커 내에서 빌드를 진행함
- colcon build 실행 중에 Killed 메세지가 뜨면서 실패 메세지 뜸 
- → 도커 컴테이너가 사용할 수 있는 Ram이 부족해서 커널이 빌드 프로세스 강제 종료한거 
- 동시 빌드 개수를 줄여서 해결 시도 
```
colcon build --symlink-install --parallel-workers 1
```
nav2_constmap_2d 빌드 실패 

- 빌드할 때에만 도커 메모리 공간 늘려주기 4 → 16G
- 동시 빌드 개수는 최대 2개로 제한 
...
논문 그래프 보면 ros tracing 사용하길래 나도 사용해봄 

32 코어로는 fast랑 cyclon 차이가 안보이는거 같아서 2코어로 cgroup만들어봄 


ros tracing 왤케 어려워


1. 그냥 ros tracing 명령어 치면 커널 정보가 누락되서 녹화됨 
sol : ros2 trace --session-name my_session_name2 --kernel sched_switch --ust ros2:rclcpp_publish ros2:*

2. 분명 uid랑 커널 녹화 다 했는데 ros2 incubator 가 열리지 않음 
이유 : gazebo, nav2 미리 실행하고 tracing 녹화했더니 인식 못한거였음 

sol : 녹화 시작 - gazebo, nav2 열기 - 로봇 이동 - 녹화 끝 

3. ros2 message control이 안열림 
이유 : 기록할 양이 너무 많아서 누락됨 

sol : 버퍼 크기 늘려주기 
ros2 trace --session-name fastdds_final_clean -b 8192 --kernel sched_switch --ust ros2:*
...
ros tracing 하루종일 잡는거 좀 아닌거 같아서 ps로 보는걸로 



## 또 다시 실험 환경 세팅

FastDDS의 'Zero-Copy' 기능이 CPU를 아껴주는 건 오피셜임

근데 평소엔 fast가 더 성능이 좋아도 혹시 네트워크 상태가 안 좋아지면, 주변 상황이 불안해지면 그 복잡한 기능 때문에 오히려 더 성능이 떨어지지 않을까?

자원을 아끼기 위해 zero copy 기법을 사용해도 다른 외부 변수로 인해 오히려 성능 저하?


**가설**
- 이상적 환경 (Ideal): 대역폭이 큰 이미지 데이터를 처리할 때, Shared Memory(SHM)를 사용하는 FastDDS가 CPU 오버헤드가 적어 성능이 우수할 것이다.
- 악조건 환경 (Adverse): 네트워크 패킷 손실이 발생하거나 CPU 자원이 극도로 제한될 경우, 복잡한 기능(Discovery/SHM 관리)이 많은 FastDDS보다 구조가 단순한 CycloneDDS가 성능 저하폭이 적고(Predictable), 시스템 안정성이 높을 것이다.


**실험 환경 및 도구 (Methodology)**

- 소프트웨어 및 하드웨어 환경

운영체제 (OS): Ubuntu 22.04 LTS (Jammy Jellyfish)

커널 버전 (Kernel): Linux Kernel 5.15 Generic (cgroup v2 및 tc-netem 지원)

미들웨어 (Middleware): ROS 2 Humble Hawksbill

시뮬레이터: Gazebo Classic 11

타겟 로봇: TurtleBot3 Waffle Pi

    센서 구성: 2D LiDAR (LDS-01) + RGB Camera (Intel RealSense R200 모사)

    선정 이유: LiDAR의 저용량 데이터와 Camera의 고대역폭(High-bandwidth) 데이터를 동시에 처리하는 복합 부하 상황을 연출하기 위함.


- 로봇 모델: TurtleBot3 Waffle Pi (LiDAR + Camera 사용)

- 시뮬레이터: Gazebo (Localhost)

- 부하 조건 (Stress):

    Traffic: ros2 run image_tools showimage (카메라 영상 구독으로 대역폭 포화 유도)

    Resource: cgroup 또는 taskset을 사용하여 CPU 코어 수 및 사용량(Quota) 제한

    Network: tc (Traffic Control)를 사용하여 인위적인 지연(Delay) 및 패킷 손실(Loss) 주입

- 분석 도구: 자체 개발한 Python Monitoring Script (Application-level Instrumentation) + ros2 topic hz_
...
- 시스템 안정성 (System Stability):
    ...
    Nav2 제어 주기 (/cmd_vel Hz): 20Hz 방어율

    Odometry 주기 (/odom Hz): 끊김 여부

- 반응성 (Latency & Jitter):

    Python 스크립트로 측정한 End-to-End Latency의 평균 및 표준편차(Jitter)


**연구의 한계점 (Limitations to Acknowledge)**

본 연구는 다음과 같은 한계를 가지며, 결과 해석 시 이를 고려해야 함.

Loopback의 한계: tc로 모사한 네트워크 환경은 실제 무선(Wi-Fi) 환경의 물리적 간섭(Interference)과 완전히 동일하지 않음.

하드웨어 아키텍처 차이: Desktop PC의 캐시/메모리 성능이 실제 임베디드 보드보다 우수하므로, 병목 현상이 완화되어 관측될 수 있음.

인과관계의 불확실성: OS 스케줄러 등 외부 요인을 완벽히 통제하지 못했으므로, 성능 변화가 오직 DDS 탓이라고 단정할 수 없으며 '경향성(Trend)' 위주로 분석함.
...
실험 설계가 너무 '낙관적(Optimistic)'

OS 연구는 **'최악의 상황(Worst Case)'**을 가정하고, 보이지 않는 커널 내부의 움직임을 추론하는 과정이어야 해. 지금 가서:

    CPU Quota 설정해서 속도 제한하고, (cpu.cfs_quota_us 값 세팅 )

    Loopback MTU 1500으로 낮추고,  (가능하다면 Loopback의 MTU를 1500으로 강제로 낮춰서(sudo ip link set lo mtu 1500) 실험 )

    User/Sys 타임 쪼개서 측정하고, (어디서 오버헤드가 발생했는지가 중요함 )

    프로세스를 한 코어에 몰아넣고(Pinning) (방해할 수밖에 없는 상황을 만들어야함)
